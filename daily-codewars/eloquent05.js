...
var SCRIPTS = [
  ...
  function dominantDirection(text) {
    let listOfScripts = textScripts(text).split(',')
    let mayorityScript = 0
    let biggest = 0
  
    listOfScripts =  listOfScripts.map(el => {
      return el.split('%')
    })
  
    listOfScripts.forEach((el, i) => {
      if(Number(el[0].trim()) > biggest){
        biggest = Number(el[0].trim())
        mayorityScript = i
      }
    })
  
    for (let script of SCRIPTS) {
      //console.log(listOfScripts[mayorityScript][1])
      if(script.name === listOfScripts[mayorityScript][1].trim()){
